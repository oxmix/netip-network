...
func connect(cp *connectPayload) *connectResponse {
	...
	res, err := connectHttpClient.Do(req)
	if err != nil {
		return connectFatal(err)
	}
	...
	if api.EndpointIP == "" {
		...
		dialer := websocket.Dialer{
			...
			NetDialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
				...
				if err != nil {
					log.Println("[connection] parse addr dial, err:", err)
				} else {
	...
	for {
		...
		log.Println("[connection] handshake successful")
		return api
	}
}

func connectFatal(err error) *connectResponse {
	log.Println("[connection] fatal: shutting down in 5 sec., handshake request err:", err)
...
func connectDegrade(err error) *connectResponse {
	...
	wsConnect = nil
	log.Println("[connection] failure, err:", err)
	if strings.Contains(err.Error(), "number of nodes has been reached") {
		log.Println("[connection] trying to reconnect after waiting 5 min.")
		time.Sleep(5 * time.Minute)
	} else {
		log.Println("[connection] trying to reconnect after waiting 5 sec.")
		time.Sleep(5 * time.Second)
	}
	return connect(nil)
}
