	...
	go func() {
		for {
			...
			case "ping-ips-refresh":
				log.Println("refreshing ping pool")
	...
	for {
		...
		case cn, ok := <-col.ChanNetwork:
			...
			if err != nil {
				...
				connectDegrade(errors.New("chan network write err: " + err.Error()))
				continue
			}
		...
		case wgs, ok := <-col.ChanWgStats:
			...
			err := wsConnect.WriteJSON(j)
			if err != nil {
				log.Println("wg stats write err:", err)
				continue
			}
		...
		case prt, ok := <-col.ChanPingRTT:
			...
			err := wsConnect.WriteJSON(j)
			if err != nil {
				log.Println("ping rtt write err:", err)
				continue
			}
