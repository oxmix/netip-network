...
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o network .

FROM docker.io/library/golang:1.22-alpine AS coredns
WORKDIR /app
...
COPY --from=oxmix/proxy:latest /proxy-nginx .
