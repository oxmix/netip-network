...
func (f *Firewall) Refresh(rules []FirewallRules) {
	...
	for _, e := range rules {
		...
		if protocol == "icmp" {
			...
			f.shell("nft add rule inet " + f.table + " input " + source + " meta l4proto { icmp, ipv6-icmp } counter accept")
			f.shell("nft insert rule ip nat " + f.table + " " + source + " meta l4proto { icmp, ipv6-icmp } counter return")
			continue
		}
